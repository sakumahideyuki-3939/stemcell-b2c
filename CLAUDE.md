...
---
## 次回セッション指示書 - clinic/ コンテンツ実装

### 前提
- グリッドレイアウトは絶対に崩さない（u-unit統一・50%正方形・フッター25%は確定済み）
- 薬機法遵守：効能効果表現禁止・「研究用試薬」前提の文言のみ

### 実装タスク（順番通りに実行）

1. clinic/index.php のヒーローセクションにコピーを入れる
   「培養上清液の、その先へ。特許4件取得の幹細胞生搾り濾液。」

2. 行1-L(u-unit)：上清液との違いセクション
   - 生搾り vs 上澄みの説明テキスト
   - 簡易比較表（成分量・特許・Cell-nucleus Free）

3. 行1-C(u-unit)：特許4件一覧
   - 下部尿路障害・ED・変形性膝関節症・皮膚保護・卵子成熟促進・精子活性剤
   - 診療科対応付き

4. 行1-R(50%正方形)：Lysate作用機序テキスト図解

5. 行2-L(50%正方形)：ダーマペン切り替えガイド
   - STEP1: 無償サンプル申し込み
   - STEP2: 現行上清液と比較施術
   - STEP3: 導入可否をご判断

6. 行2-C(u-unit)：更年期・レディース応用（示唆のみ）

7. 行2-R(u-unit)：導入フロー（3ステップ）

8. 行3 小ブロック×8：
   成分比較 / Cell-nucleus Free / 特許一覧 / コンプライアンス /
   育毛エビデンス / PRP比較 / サンプル請求フォームへ / お問い合わせへ

### 完了後
git add -A && git commit -m "feat: clinic/ コンテンツ初期実装" && git push origin main
